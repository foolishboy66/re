...
rep = RepresentationLayer(wordvec_file=word_vec_file, frequency=200000, max_sent_len=max_len)
...
word_emb = Embedding(rep.vec_table.shape[0], rep.vec_table.shape[1],
                     weights = [rep.vec_table], mask_zero=False, input_length=max_len)
position_emb = Embedding(max_len * 2 + 1, position_vec_dim, mask_zero=False, input_length=max_len)
...
merged = merge([word_vec, distance1_vec, distance2_vec], mode='concat')
dropouted = Dropout(0.5)(merged)
